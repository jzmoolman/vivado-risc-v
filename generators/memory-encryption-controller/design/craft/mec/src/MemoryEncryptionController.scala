  ...
  class MemoryEncryptionController(
                        val mec: MemoryEncryptionControllerParameters,
...
//    var resourcesOpt: Option[ResourceBindings] = None
    ...
    val device: SimpleDevice = new SimpleDevice("memory-encryption-controller", Seq("jzm,mec0", "mec")) {
...
//        Description(name, mapping ++ extra ++ nextlevel)
        Description("name", "mappings ++ extra ++ nextlevel")
      }
    }
